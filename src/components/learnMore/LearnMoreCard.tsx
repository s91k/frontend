...
export function LearnMoreCard({ article }: { article: (typeof articleMetaData)[number] }) {
    const { t } = useTranslation();
    const { currentLanguage } = useLanguage();
  
    return (
      <Link
        to={`/${currentLanguage}/insights/${article.id}`}
      ...
      >
        ...
        <div className="p-8 space-y-4">
          ...
          <Text className="text-grey">{article.excerpt}</Text>
